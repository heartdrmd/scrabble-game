...
app = Flask(__name__)
CORS(app)
app.secret_key = os.environ.get('SECRET_KEY', 'family-game-night-2024-scrabble-fun')
...
<body>
    <div class="login-box">
        ...
        <form method="POST">
            <input type="password" name="code" placeholder="••••••" maxlength="6" autofocus inputmode="numeric" pattern="[0-9]*">
            <br>
...
def get_valid_code():
    """Generate the valid code: 9069 + 2-digit day in Central Time"""
    central = pytz.timezone('America/Chicago')
    today = datetime.now(central).day
    return f"9069{today:02d}"
...
def login():
    """Login page with dynamic code"""
    if request.method == 'POST':
        entered_code = request.form.get('code', '').strip()
        correct_code = get_valid_code()
        
        if entered_code == correct_code:
            session['authenticated'] = True
            session.permanent = True  # Keep session for 31 days by default
            return redirect(url_for('index'))
        else:
            return render_template_string(LOGIN_PAGE, error="Invalid code - try again")
